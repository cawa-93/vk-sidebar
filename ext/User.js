...
function User (user_obj) {
	...
	if (typeof user_obj === 'number') {
		...
		if (window.Sidebar.profiles[search_id]) {
			$.extend(this, window.Sidebar.profiles[search_id]);
			return true;
		} else {
			console.warn('User not loaded: ' + search_id);
			window.Sidebar.callAPI('users.get', {
				async: false,
				context: this,
				data: {
					user_ids: search_id,
					...
					window.Sidebar.profiles[search_id] = new User(API[0]);
					$.extend(true, this, new User(API[0]));
				}
			});
		...
		this.ava = function (options) {
			...
			var onlineClass = (!!this.online && options.marker ? ' online' : ''),
				imgHTML = '<div class="ava ava-' + options.type + onlineClass + '"><img title="' + (options.title ? this.name : '') + '" src="' + this.photo_50 + '" width="' + options.size + '" height="' + options.size + '"/></div>';
		...
		this.addOrDel = function (method, options) {
			...
			window.Sidebar.callAPI('friends.' + method, options);
		};
		...
		this.upData = function (user_id) {
			...
			if (window.Sidebar.profiles[user_id]) {
				$.extend(this, window.Sidebar.profiles[user_id]);
				if (this.jQ) {
					this.jQ.data(this);
				}
